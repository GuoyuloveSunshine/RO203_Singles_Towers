...
function readInputFile(inputFile::String)
    ...
    close(datafile)
    lenData = length(split(data[1],","))
    lenValid = lenData - 2

    ## Create a 2-dimensional array of size n.
    t = Array{Int64}(undef,lenValid,lenValid)

    ## Create the visibility vector.
    vL = Vector{Int64}(undef,lenValid)
    vR = Vector{Int64}(undef,lenValid)
    vU = Vector{Int64}(undef,lenValid)
    vD = Vector{Int64}(undef,lenValid)

    ## For each line of the input file.
    for line in 1:lenData
        lineSplit = split(data[line],",")
        if line == 1
            for col in 2:lenData-1
                vU[col-1] = parse(Int64,lineSplit[col])
            end
            
        elseif line == lenData
            for col in 2:lenData-1
                vD[col-1] = parse(Int64,lineSplit[col])
            end
        else
            vL[line-1] = parse(Int64,lineSplit[1])
            for col in 2:lenData-1
                t[line-1,col-1] = parse(Int64,lineSplit[col])
            end
            vR[line-1] = parse(Int64,lineSplit[lenData])
        end
    end
    return t, vL, vR, vU, vD
end
...
Arguments:
    - t: 2-dimensional array of size (n+2) * (n+2)
...
Arguments:
    ...
    - xk: array of size n*n*n (xk[i, j, k] = 1 if cell (i, j) has value k)
"""
function writeSolution(fout::IOStream, xk::Array{VariableRef, 3})
    n = size(xk, 1)
    x = Array{Int64}(undef, n, n)

    ## Convert the solution from xk[i, j, k] variables into x[i, j] variables
    for l in 1:n
        for c in 1:n
            for k in 1:n
                if JuMP.value(xk[l, c, k]) > TOL
...
"""
function performanceDiagram(outputFile::String = "../res/jeu2.png")
...
- Each text file contains a variable "solveTime" and a variable "isOptimal"
"""
function resultsArray(outputFile::String = "../res/array_jeu2.tex")
